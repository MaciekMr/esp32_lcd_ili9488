...
void     LCD_Delay (uint32_t delay); //Delay in ms
void     LCD_IO_Init(void); //Initialise SPI interface
void     LCD_IO_Bl_OnOff(uint8_t Bl); //Switch on/off the backlight 

void     LCD_IO_WriteCmd8(uint8_t Cmd);  //SPI write command (before send data via SPI set the pin D/C to low and CS (chip select))
void     LCD_IO_WriteData8(uint8_t Data); //SPI write data (before send  set the pin D/C to high  and CS (chip select))
void     LCD_IO_WriteData16(uint16_t Data); //Switch SPI to 16bit transmission and send data  with DMA, before send wait until DMA free)
